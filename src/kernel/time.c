...
void time_read_bcd(tm *time)
{
    ...
    do
    {
        time->tm_sec = cmos_read(CMOS_SECOND_READ);
        time->tm_min = cmos_read(CMOS_MINUTE_READ);
        time->tm_hour = cmos_read(CMOS_HOUR_READ);
        time->tm_wday = cmos_read(CMOS_WEEKDAY_READ);
        time->tm_mday = cmos_read(CMOS_DAY_READ);
        time->tm_mon = cmos_read(CMOS_MONTH_READ);
        time->tm_year = cmos_read(CMOS_YEAR_READ);
        century = cmos_read(CMOS_CENTURY_READ);
    } while (time->tm_sec != cmos_read(CMOS_SECOND_READ));
}
...
void time_init()
{
    ...
    LOGK("[INFO] time: %d%d-%02d-%02d %02d:%02d:%02d\n",
         century,
         time.tm_year,
         time.tm_mon,
         time.tm_mday,
