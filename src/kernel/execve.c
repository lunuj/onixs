...
int sys_execve(char *filename, char *argv[], char *envp[])
{
    ...
    for (size_t i = 0; i < ehdr->e_phnum; i++)
    {
        memset(content, 0, MEMORY_PAGE_SIZE);
    ...
    {
        LOGK("v: %#x size: %d sec: %d b: %d t: %d n: %s\n",
             symptr->st_value,
             symptr->st_size,
             symptr->st_shndx,
    ...
    free_kpage((uint32)ehdr, 1);
    free_kpage((uint32)phdr, 1);
    free_kpage((uint32)content, 1);
    free_kpage((uint32)strtab, 1);
    free_kpage((uint32)symtab, 1);
    free_kpage((uint32)shstrtab, 1);
